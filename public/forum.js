      ...
      .then(categories => {
          categories.forEach(category => {
              ...
              categoriesList.addEventListener('click', function(event) {
                ...
                if (clickedCategory) {
                    const categoryId = category.postId;
                    const categoryTitle = clickedCategory.querySelector('.category-title').innerText;
                    window.location.href = `/threads?category=${categoryId}&title=${encodeURIComponent(categoryTitle)}`;
                }
            });
